...
export async function execute(interaction: ChatInputCommandInteraction) {
  const profile = await kvService.getUserProfile(interaction.user.id);
...
export async function handleRulesAgreement(interaction: ButtonInteraction) {
  try {
    ...
    let profile = await kvService.getUserProfile(interaction.user.id);
    ...
    await kvService.setUserProfile(interaction.user.id, profile);
