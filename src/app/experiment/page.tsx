...
const ProjectWidget = ({ title, difficulty, bpm, index }: any) => {
  // Stagger the indentation to create the "arc" effect
  const marginRight = `${index * 20}px`;

  return (
    <div 
      style={{ animationDelay: `${index * 0.1}s`, marginRight }}
      className={`
        animate-osu-item group relative h-20 w-[400px] cursor-pointer transition-all duration-200
        hover:scale-105 hover:z-10
      `}
    >
      {/* Step 1: The Slanted Background */}
      <div className="osu-slant absolute inset-0 bg-slate-800 border-r-8 border-pink-500 group-hover:bg-pink-600 transition-colors" />

      {/* Step 2: The Content */}
      <div className="relative flex items-center h-full px-12 text-white">
        <div className="flex-1">
          <h3 className="text-xl font-black italic uppercase tracking-tighter">
            {title}
          </h3>
          <div className="flex gap-3 text-xs opacity-80">
            <span className="bg-black/40 px-2 rounded">BPM: {bpm}</span>
            <span className="text-yellow-400 font-bold">★ {difficulty}</span>
          </div>
        </div>

        {/* Step 3: The Rank/Grade (Right side) */}
        <div className="text-4xl font-black italic opacity-20 group-hover:opacity-100 transition-opacity">
          S
        </div>
      </div>
    </div>
